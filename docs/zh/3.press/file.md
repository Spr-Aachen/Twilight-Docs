...
> ⚠️ 单文件方案会导致RSS无法正常构建图片的路径(指本地,如果你使用图床那么不会有这个问题),如果你需要使用rss功能请使用文件夹写作方案
...
### 复制保护
- `copyProtection`：对象类型，用于启用页面级别的复制保护功能。每个子选项默认为 `false`：
  - `blockSelection`：禁止选中文本（`user-select: none`）
  - `blockClipboard`：禁止复制、剪切、粘贴事件
  - `blockContextMenu`：禁止右键菜单
  - `blockDevTools`：禁止 F12、Ctrl+U（查看源代码）、Ctrl+S（保存页面）
